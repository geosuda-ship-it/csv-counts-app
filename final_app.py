...
st.set_page_config(
    page_title="STEP 3　判定結果を集計し、各サンプルの最終判定を行う",
    page_icon="🔬",
    layout="centered",
)

st.title("STEP 3　判定結果を集計し、各サンプルの最終判定を行う")
st.write(
    "STEP 2で作成した判定結果ファイルと、"
    "共通のグループ集計シートをアップロードすると、"
    "サンプルごとの最終判定結果と化学組成グループの集計結果を"
    "Excelファイルとしてダウンロードできます。"
)
