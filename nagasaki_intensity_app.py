...
from colab_edx_full_pipeline import (
    QC_MODE,
    calculate_all,
    convert_csv,
...
st.set_page_config(
    page_title="Rigaku NEX DE 強度・定量変換ツール",
    page_icon="🧪",
    layout="wide",
)
...
st.markdown(
    """
    <div class="app-title">
    Rigaku NEX DE用 強度から定量値への変換ツール（長崎大学版）
    </div>
...
with st.expander("計算内容", expanded=False):
    st.markdown(
        ...
        - K、Ca、Mn、Fe：ドリフト補正後強度を使用
        - Zn、Rb、Sr、Y、Zr、Nb：ドリフト補正後Ag強度で内標準化
...
if st.button("計算を実行", type="primary", use_container_width=True):
    try:
        with st.spinner("補正係数・補正後強度・定量値を計算しています…"):
            # 入力検証も兼ねて全計算を実行する。
        ...
        st.session_state.skipped_dates = skipped_dates
    except Exception as exc:
        clear_previous_result()
...
if st.session_state.get("result_bytes") is not None:
    st.success(
        f"計算が完了しました。"
        f"{st.session_state.get('result_count', 0)}行を出力します。"
    )

    skipped_dates = st.session_state.get("skipped_dates", [])
    if skipped_dates:
        st.warning(
            "同日のQCがなく除外した測定日："
